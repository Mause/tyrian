...
import os
co_flags = {}
with open(os.path.join(os.path.dirname(__file__), 'code.h')) as fh:
    for name, value in re.findall(r'#define (CO_\w+) +(0x\d+)', fh.read()):
        co_flags[name] = int(value, 16)
globals().update(co_flags)

__all__.extend([k for k in list(globals().keys()) if k.startswith('CO_')])
...
def Local(name, code=None):
    ...
    elif code.co_flags & co_flags['CO_OPTIMIZED']:
...
def YieldStmt(value=None, code=None):
    ...
    if stack_effects[opmap['YIELD_VALUE']][1]:
        code.POP_TOP()
    return r
...
class Label(object):
    ...
    def SETUP_EXCEPT(self, code):
        code.SETUP_EXCEPT()
        self.backpatches.append(code.blocks[-1][-1])
    ...
    def __call__(self, code):
        ...
        self.resolution = len(code.__code__)
...
class Code(object):
    co_argcount = 0
    co_stacksize = 0
    co_flags = co_flags['CO_OPTIMIZED'] | co_flags['CO_NEWLOCALS']      # typical usage
    ...
    def __init__(self):
        self.__code__ = array('B')
        self.co_consts = [None]
        self.co_names = []
        self.co_varnames = []
        self.co_lnotab = array('B')
        self.emit = self.__code__.append
        self.blocks = []
        self.stack_history = []
    ...
    def locals_written(self):
        vn = self.co_varnames
        hl = dict.fromkeys([opmap['STORE_FAST'], opmap['DELETE_FAST']])
    ...
    def set_lineno(self, lno):
        ...
        incr_addr = len(self.__code__) - self._last_lineofs
        if not incr_line:
            return
        ...
        self._last_line = lno
        self._last_lineofs = len(self.__code__)

    def YIELD_VALUE(self):
        self.stackchange(stack_effects[opmap['YIELD_VALUE']])
        self.co_flags |= co_flags['CO_GENERATOR']
    ...
    def set_stack_size(self, size):
        ...
        bytes = len(self.__code__) - len(self.stack_history) + 1
        if bytes > 0:
    ...
    def branch_stack(self, location, expected):
        if location >= len(self.stack_history):
            if location > len(self.__code__):
                raise AssertionError("Forward-looking stack prediction!",
                                     location, len(self.__code__)
                                     )
    ...
    def jump(self, op, arg=None):
        ...
        if op == opmap['FOR_ITER']:
            old_level = self.stack_size = self.stack_size - 1
            self.stack_size += 2
        else:
            old_level = self.stack_size
        self.stack_size -= (op in (opmap['JUMP_IF_TRUE_OR_POP'], opmap['JUMP_IF_FALSE_OR_POP']))
        posn = self.here()
        if arg is not None:
            ...
            def lbl(code=None):
                backpatch(self.here())
        if op in (opmap['JUMP_FORWARD'], opmap['JUMP_ABSOLUTE'], opmap['CONTINUE_LOOP']):
            self.stack_unknown()
        return lbl

    def COMPARE_OP(self, op):
        self.stackchange((2, 1))
        self.emit_arg(opmap['COMPARE_OP'], compares[op])
    ...
    def SETUP_EXCEPT(self):
        ...
        self.setup_block(opmap['SETUP_EXCEPT'])
    ...
    def SETUP_FINALLY(self):
        ...
        self.setup_block(opmap['SETUP_FINALLY'])
        self.stack_size = ss    # restore original level

    def SETUP_LOOP(self):
        self.setup_block(opmap['SETUP_LOOP'])
    ...
    def POP_BLOCK(self):
        ...
        self.emit(opmap['POP_BLOCK'])

        if why != opmap['SETUP_LOOP']:
            if why == opmap['SETUP_FINALLY']:
    ...
    if 'LIST_APPEND' in opcode and opmap['LIST_APPEND'] >= HAVE_ARGUMENT:
        def LIST_APPEND(self, depth):
            self.stackchange((depth + 1, depth))
            self.emit_arg(opmap['LIST_APPEND'], depth)
    ...
    def BREAK_LOOP(self):
        self.assert_loop()
        self.emit(opmap['BREAK_LOOP'])
        self.stack_unknown()

    def CONTINUE_LOOP(self, label):
        self.assert_loop()
        if self.blocks[-1][0] == opmap['SETUP_LOOP']:
            op = opmap['JUMP_ABSOLUTE']  # more efficient if not in a nested block-
        else:
            op = opmap['CONTINUE_LOOP']
    ...
    def from_spec(cls, name='<lambda>', args=(), var=None, kw=None):
        ...
        if var:
            ...
            self.co_flags |= co_flags['CO_VARARGS']
        if kw:
            self.co_varnames.append(kw)
            self.co_flags |= co_flags['CO_VARKEYWORDS']
    ...
    def patch_arg(self, offset, oldarg, newarg):
        code_obj = self.__code__
        if (oldarg > 0xFFFF) != (newarg > 0xFFFF):
            raise AssertionError("Can't change argument size", oldarg, newarg)
        code_obj[offset + 1] = newarg & 255
        code_obj[offset + 2] = (newarg >> 8) & 255
        if newarg > 0xFFFF:
            newarg >>= 16
            code_obj[offset - 2] = newarg & 255
            code_obj[offset - 1] = (newarg >> 8) & 255

    def nested(self, name='<lambda>', args=(), var=None, kw=None, cls=None):
        if cls is None:
            cls = Code
        code_obj = cls.from_spec(name, args, var, kw)
        code_obj.co_filename = self.co_filename
        return code_obj
    ...
    def __iter__(self):
        ...
        while i < n:
            op = code_obj[i]
            if op >= HAVE_ARGUMENT:
                oparg = code_obj[i + 1] + code_obj[i + 2] * 256 + extended_arg
                extended_arg = 0
                if op == opmap['EXTENDED_ARG']:
    ...
    def makecells(self, names):
        ...
        if newcells:
            if not (self.co_flags & co_flags['CO_OPTIMIZED']):
        ...
        for ofs, op, arg in self:
            if op in opmap:
                if arg in argmap:
                    ...
                    continue
                code_obj[ofs] = opmap[op]

    def code(self, parent=None):
        if self.blocks:
            raise AssertionError("%d unclosed block(s)" % len(self.blocks))

        flags = self.co_flags & ~co_flags['CO_NOFREE']
        if parent is not None:
            locals_written = self.locals_written()
            self.makefree([
                n for n in self.co_varnames[
                    self.co_argcount
                    + ((self.co_flags & co_flags['CO_VARARGS']) == co_flags['CO_VARARGS'])
                    + ((self.co_flags & co_flags['CO_VARKEYWORDS']) == co_flags['CO_VARKEYWORDS']):
                ] if n not in locals_written
            ])

        if not self.co_freevars and not self.co_cellvars:
            flags |= co_flags['CO_NOFREE']
        elif parent is not None and self.co_freevars:
            parent.makecells(self.co_freevars)

        self.co_kwonlyargcount = 0

        # refer to code_new in Objects/codeobject.c in cpython source for order
        return CodeType(
            int(self.co_argcount),
            self.co_kwonlyargcount,
            len(self.co_varnames),
            int(self.co_stacksize),
            int(flags),
            bytes(self.__code__.tostring()),
            tuple(self.co_consts),
            tuple(self.co_names),
            tuple(self.co_varnames),
            str(self.co_filename),
            str(self.co_name),
            int(self.co_firstlineno),
            bytes(self.co_lnotab.tostring()),
...
for op in hasname:
    if not hasattr(Code, opname[op]):
        def do_name(self, name, op=op):
            ...
            self.emit_arg(op, arg)
            if op in (opmap['LOAD_NAME'], opmap['STORE_NAME'], opmap['DELETE_NAME']):
                # Can't use optimized local vars, so reset flags
                self.co_flags &= ~co_flags['CO_OPTIMIZED']
...
def iter_code(codestring):
    ...
    while ptr < size:
        ...
        if op >= HAVE_ARGUMENT:
            arg = ord(
                codestring[ptr]) + ord(codestring[ptr + 1]) * 256 + extend
            # extended_arg = 0
            ptr += 2
            if op == opmap['EXTENDED_ARG']:
                extend = arg * 65536
...
def dump(code):
    ...
    labels = {}
    instructions = list(iter_code(code.__code__))
    lbl = [jump for start, op, arg, jump,
           end in instructions if jump is not None]
    lbl.sort()
    for jump in lbl:
    ...
    while i < len(instructions):
        ...
        print('       ', labels.get(start, '').ljust(7), end=' ')
        if op == opmap['DUP_TOP'] and instructions[i + 1][1] in (opmap['POP_JUMP_IF_FALSE'], opmap['POP_JUMP_IF_TRUE']):
            s, op, arg, jump, end = instructions[i + 1]
            print(['JUMP_IF_FALSE', 'JUMP_IF_TRUE'][
                  op == opmap['POP_JUMP_IF_TRUE']].ljust(15), end=' ')
            i += 1
        elif op in (opmap['JUMP_IF_TRUE_OR_POP'], opmap['JUMP_IF_FALSE_OR_POP']):
            print(opname[op][:-7].ljust(15), end=' ')
        else:
            print(opname[op].ljust(15), end=' ')
        if jump is not None:
            print(labels[jump][:-1].rjust(10), end=' ')
        elif arg is not None:
            print(repr(arg).rjust(10), end=' ')
            if op in argtype:
                print('(%s)' % (locals()[argtype[op]][arg]), end=' ')
        print()
        if op in (opmap['JUMP_IF_TRUE_OR_POP'], opmap['JUMP_IF_FALSE_OR_POP']):
